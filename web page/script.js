...
if (window.location.pathname.includes('create.html')) {
  ...
  if (createBtn) {
    createBtn.addEventListener('click', () => {
      notify('製作功能開發中 — 僅供教學用途，請勿用於不法用途。', 4000);
    });
  }
}
...
function notify(message, duration = 3000) {
  // 建立一個暫時性的通知容器
  const containerId = 'site-notify-container';
  ...
  setTimeout(() => {
    ...
    setTimeout(() => container.removeChild(note), 300);
  }, duration);
}